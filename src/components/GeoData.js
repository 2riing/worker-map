...
import wrkrOrderData from '../wrkrOrderData';
    ...
    useEffect(() => {
        const fetchGeocodeData = async () => {
            try {
                ...
                const requests = wrkrOrderData.map(worker =>
                    axios.get(`/map-geocode/v2/geocode?query=${encodeURIComponent(worker.address)}`, {
                        headers: {
